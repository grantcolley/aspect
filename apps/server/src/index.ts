...
if (!process.env.ENDPOINT_CATEGORIES) {
  throw new Error("ENDPOINT_CATEGORIES environment variable is not set");
}

if (!process.env.ENDPOINT_MODULES) {
  throw new Error("ENDPOINT_MODULES environment variable is not set");
}
...
const categoriesEndpoint = process.env.ENDPOINT_CATEGORIES;
const modulesEndpoint = process.env.ENDPOINT_MODULES;
...
const start = async () => {
  ...
  app.use(categoriesEndpoint, categoriesRouter);
  app.use(modulesEndpoint, modulesRouter);
